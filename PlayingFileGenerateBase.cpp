...
bool PlayingFileGenerateBase::exportScript700(const char *path, const RegisterLogger &reglog)
{
	...
	fprintf(fp, "; This file was generated by C700.\n");
	...
	for (int i=0; i<reglog.mLogCommandsPos; i++) {
		...
		requiedWait = 0;
		if (waitTime > 0) {
			fprintf(fp, "%02x%02x%02x%02x", waitTime & 0xff, (waitTime >> 8) & 0xff, (waitTime >> 16) & 0xff, (waitTime >> 24) & 0xff);
			outDataBytes += 4;
			waitTime = 0;
		}
		if (i == reglog.mLogCommandsLoopPoint) {
			loopPointPos = outDataBytes;
		}
		unsigned char cmd = reglog.m_pLogCommands[i].data[0];
		int cmdLen = getCommandLength(cmd);
		if (cmd < 0x80) {
			if (cmdLen == 2) {
				fprintf(fp, "%02x%02x", cmd, reglog.m_pLogCommands[i].data[1]);
				outDataBytes += 2;
				...
				fprintf(fp, "%02x%02x", cmd-1, reglog.m_pLogCommands[i].data[2]);
				outDataBytes += 2;
				fprintf(fp, "%02x%02x%02x%02x", normal_wait & 0xff, (normal_wait >> 8) & 0xff, (normal_wait >> 16) & 0xff, (normal_wait >> 24) & 0xff);
				outDataBytes += 4;
				fprintf(fp, "%02x%02x", cmd, reglog.m_pLogCommands[i].data[1]);
				outDataBytes += 2;
